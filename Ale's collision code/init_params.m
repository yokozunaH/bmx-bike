...
function params = init_params
    % parameters that appear in the dynamics:
     ...
     params.model.dyn.wheel_res = 0; %resitution of wheels for impulse calculation
     params.model.dyn.collision_threshold = 0.001; %Threshold to stop collision detection
    ...
    params.model.ramp_up_start = 0.5; %time to start accelerating
    params.model.ramp_up_end = 1.5; %time to stop accelerating
...
%     params.model.geom.fw_com.theta = 2.464; %Initial angle between back wheel and CoM
    ...
    params.viz.axis_lims = [-0.5,10,-0.1,4];
    params.viz.dt = 0.05;
    ...
    params.sim.trick = 'Backflip';
     ...
     % variables related to the constraints
    % list of *unilateral* constraints: [bw_com_x bw_y, fw_y bw_fw_x,bw_ramp,fw_ramp]   1 if active; 0 if inactive
    %initially, bw and fw constraint to the ground and spinning at the same speed

%     switch params.sim.trick 
%         
%         case 'Wheelie'
%             params.sim.constraints.active = [1, 1, 1]; %don't include ramp constarints when attempting a wheelie
%         case 'Backflip'
%             params.sim.constraints.active = [1 1 1 1 0 0];   
%     end
