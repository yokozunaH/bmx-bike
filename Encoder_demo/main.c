...
int main(void)
{
    ...
    //
	QEIConfigure(QEI0_BASE, (QEI_CONFIG_CAPTURE_A_B  | QEI_CONFIG_RESET_IDX | QEI_CONFIG_QUADRATURE | QEI_CONFIG_NO_SWAP), 8192);  // why max 1024?? need to configure?
    ...
    ConfigureUART();
        ...
        while(1)
        {
            ...
            float pos_f = pos*0.049453; // (8192 pulses per revolution)
            UARTprintf("pos = %d degrees\n", (int) pos_f);//0.049453);
